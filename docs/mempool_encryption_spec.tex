...
\begin{itemize}
  ...
  \item $\mathsf{AEAD}$ be an authenticated encryption scheme (e.g.\ AES-256-GCM or ChaCha20-Poly1305).
\end{itemize}

All group elements are serialized using the standard compressed encodings for BLS12-381.
The security parameter is $\lambda = 256$. The HKDF salt is the fixed protocol constant
...
\begin{enumerate}
  \item Sample a random symmetric key $K \in \{0,1\}^{256}$.
  ...
  K'=\mathsf{HKDF\text{-}Expand}(\mathrm{PRK},\, \texttt{DST}_{\mathsf{kdf}} \parallel \mathsf{enc}(\mathsf{tg},U,PK),\, 32).
  \]
...
\begin{itemize}
  \item \textbf{Confidentiality:} The KEM is IND-CPA secure under the decisional bilinear
        Diffie--Hellman (DBDH) assumption in $(G_1,G_2,G_T)$, modeled in the random oracle model
        for $H_{\mathsf{BLS\_sig}}$ and HKDF.
